...
class Window:
    def __init__(self):
        ...
        self.window = pg.display.set_mode((WIDTH, HEIGHT))  # init window
...
class Settings:
    ...
    def settings_elements(self):
        ...
        self.window.blit(text_size_text, (WIDTH // 8, HEIGHT // 8 + HEIGHT // 20))
        rows_text = self.font.render("Rows in matrix is " + "{}".format(ROWS), 1, COLOR_GAME_OVER)
        self.window.blit(rows_text, (WIDTH // 8, HEIGHT // 8 + HEIGHT // 10))
...
class Food:
    def __init__(self, window):
        ...
        self.ate = 0
        self.score = 0
        self.eat_food = pg.mixer.Sound('sounds/eat_food.ogg')
        self.eat_food.set_volume(1)
