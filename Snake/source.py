...
LENGTH = config.getint("game", "resolution")
ROWS = config.getint("game", "rows")
FPS = config.getint("game", "fps")
TEXT_SIZE = LENGTH // 15
COLOR_SNAKE = tuple(map(int, config.get("color", "color_snake").split()))
COLOR_GROUND = tuple(map(int, config.get("color", "color_ground").split()))
COLOR_LINE = tuple(map(int, config.get("color", "color_line").split()))
COLOR_FOOD = tuple(map(int, config.get("color", "color_food").split()))
COLOR_MENU = tuple(map(int, config.get("color", "color_menu").split()))
COLOR_PLAY = tuple(map(int, config.get("color", "color_play").split()))
COLOR_SETTINGS = tuple(map(int, config.get("color", "color_settings").split()))
COLOR_SNAKE_GAME = tuple(map(int, config.get("color", "color_snake_game").split()))
COLOR_UNPRESSED_MENU = tuple(map(int, config.get("color", "color_unpressed_menu").split()))
COLOR_PRESSED_MENU = tuple(map(int, config.get("color", "color_pressed_menu").split()))
COLOR_GAME_OVER = tuple(map(int, config.get("color", "color_game_over").split()))
COLOR_GAME_OVER_BACK = tuple(map(int, config.get("color", "color_game_over_back").split()))
COLOR_VICTORY = tuple(map(int, config.get("color", "color_victory").split()))
COLOR_TEXT_SETTINGS = tuple(map(int, config.get("color", "color_text_settings").split()))
COLOR_TEXT_SETTINGS_S = tuple(map(int, config.get("color", "color_text_settings_s").split()))
...
class Menu:
    ...
    def draw_buttons(self):
        ...
        """
        self.screen.blit(self.main_text, ((LENGTH - LENGTH // 2.35) // 2, LENGTH // 5))
        self.screen.blit(self.play_text, (LENGTH3, LENGTH3))
        self.screen.blit(self.settings_text, (LENGTH3, LENGTH2))
    ...
    def check_m_on_play(self):
        ...
        pg.draw.rect(self.screen, COLOR_UNPRESSED_MENU,
                     (LENGTH3, LENGTH3, LENGTH35, LENGTH7))

        if (LENGTH3 <= self.mouse[0] <= (LENGTH3 + LENGTH35)) \
                and (LENGTH3 <= self.mouse[1] <= (LENGTH3 + LENGTH7)):
    ...
    def check_m_on_settings(self):
        ...
        """
        pg.draw.rect(self.screen, COLOR_UNPRESSED_MENU,
                     (LENGTH3, LENGTH2, LENGTH35, LENGTH7))

        if LENGTH3 <= self.mouse[0] <= LENGTH3 + LENGTH35 \
                and LENGTH2 <= self.mouse[1] <= LENGTH2 + LENGTH7:
            pg.draw.rect(self.screen, COLOR_PRESSED_MENU,
                         (LENGTH3, LENGTH2, LENGTH35, LENGTH7))
            if self.click[0] == 1:
...
class Settings:
    def __init__(self, wn):
        ...
        self.font_mark = pg.font.SysFont('comicsansms', TEXT_SIZE // 2)
    ...
    def check_m_on_back(self):
        if 0 <= self.mouse[0] <= LENGTH // 5.5 \
                and 0 <= self.mouse[1] <= LENGTH // 12:
            pg.draw.rect(self.screen, COLOR_PRESSED_MENU,
                         (0, 0, LENGTH // 5.5, LENGTH // 12))
            if self.click[0] == 1:
                self.res = str(LENGTH)
                self.rows = str(ROWS)
                self.fps = str(FPS)
    ...
    def check_fps(self):
        if LENGTH8 <= self.mouse[0] <= LENGTH8 + LENGTH // 1.5 \
                and LENGTH3 <= self.mouse[1] <= LENGTH3 + TEXT_SIZE:
            if self.click[0] == 1:
                self.c_f = True
    ...
    def check_apply(self):
        if LENGTH // 3.1 <= self.mouse[0] <= LENGTH3 + LENGTH // 4.5 and LENGTH * 0.9 <= \
                self.mouse[1] <= LENGTH * 0.9 + LENGTH8:
            pg.draw.rect(self.screen, COLOR_PRESSED_MENU,
                         (LENGTH // 3.1, LENGTH * 0.9, LENGTH // 4.5, LENGTH8))
            if self.click[0] == 1:
                return True
        else:
            pg.draw.rect(self.screen, COLOR_UNPRESSED_MENU,
